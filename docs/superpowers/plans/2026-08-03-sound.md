...
3. Change `tail` to a constant `0.1` → `aLongerNoteDecaysMoreSlowlyAtTheSameInstant`
   must fail.
4. Delete the `guard tail > 0` → `anEnvelopeShorterThanItsOwnAttackStillFallsToTheFloor`
   must fail or produce a non-finite value.
5. Change `attack` to `0.06` → the six-millisecond assertion must fail.
