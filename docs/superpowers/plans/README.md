...
| **9** | Parking a question in the session list — ESC sets it aside instead of giving up on it, and it renders as a `.rblock` under its own row | new; §11 + a §2.3 correction | **Tasks 1–2 done** — [the plan](2026-08-06-parking-questions.md), 8 tasks; 892 tests. All three owner decisions ruled. Measured the hook protocol: the CLI blocks on a hook and shows nothing of its own for the duration, so only one party can hold a question at a time and the deadline is the *hand-back*, not a safety net. Blocks 6.7's `handBackToTerminalAfter` row, which binds to this plan's `clampedAnswer`. **Task 5 before Task 7** |
