...
`read()` at a time, which a peer that trickles bytes below the deadline can
exceed without limit — review measured 1.56s against a 200ms deadline before
this was fixed. Both loops therefore also check the wall clock, the deadline is
clamped above zero (`timeval{0,0}` means *no timeout* to `setsockopt`), and a
partial line is discarded rather than returned as a fake success.
...
public struct SocketClient: Sendable {
    ...
    public init(path: String, deadline: TimeInterval = 0.3) {
        self.path = path
        self.deadline = Swift.max(Self.floorDeadline, deadline)
    }
    ...
    /// parking forever. `deadline` is already clamped above zero, so this
    /// never asks for the "no timeout" timeval.
...
The server creates its parent directory **as 0700**, removes any stale socket
file, narrows the umask across `bind` and `chmod`s the socket to `0600` — review
measured the file as world-connectable 0755 in the window between `listen` and
`chmod`, which is long enough for a local process to answer a permission prompt.

It also bounds its reads with an absolute deadline, and sets `SO_NOSIGPIPE` on
every accepted connection: `SIGPIPE`'s default disposition terminates the
process, so without it a hook that disconnects mid-write **kills the app**.
